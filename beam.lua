...
cmd:option('-src_file', 'src_test.txt',[[Source sequence to decode (one line per sequence)]])
...
cmd:option('-output_file', 'pred.txt', [[Path to output the predictions (each line will be the
                                                    decoded sequence]])
cmd:option('-src_dict', 'lcsts-char.src.dict', [[Path to source vocabulary (*.src.dict file)]])
cmd:option('-targ_dict', 'lcsts-char.targ.dict', [[Path to target vocabulary (*.targ.dict file)]])
cmd:option('-char_dict', 'demo.char.dict', [[If using chars, path to character
                                                                vocabulary (*.char.dict file)]])

-- beam search options
cmd:option('-beam', 3,[[Beam size]])
cmd:option('-max_sent_l', 150, [[Maximum sentence length. If any sequences in srcfile are longer
...
cmd:option('-replace_unk', 1, [[Replace the generated UNK tokens with the source token that
...
function main()
    ...
    for line in file:lines() do
        ...
        -- print attention chars in source sentence
        -- local attended_chars = ""
        -- for i = 2, #attn do
        --     attended_chars = attended_chars .. ' ' .. source_str[attn[i]]
        -- end
        -- print(attended_chars)
        -- (end) print attention chars in source sentence
        if gold ~= nil then
