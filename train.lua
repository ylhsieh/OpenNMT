...
cmd:text("")
cmd:option('-data_file','lcsts-char-train.hdf5',[[Path to the training *.hdf5 file
                                               from preprocess.py]])
cmd:option('-val_data_file','lcsts-char-val.hdf5',[[Path to validation *.hdf5 file
                                                 from preprocess.py]])
cmd:option('-savefile', 'model_300x300_biLSTM_pretrainw2v', [[Savefile name (model will be saved as
                         savefile_epochX.t7 where X is the X-th epoch]])
...
cmd:option('-lr_decay', 0.9, [[Decay learning rate by this much if (i) perplexity does not decrease
                      on the validation set or (ii) epoch has gone past the start_decay_at_limit]])
cmd:option('-start_decay_at', 10, [[Start decay after this epoch]])
cmd:option('-curriculum', 0, [[For this many epochs, order the minibatches based on source
                sequence length. Sometimes setting this to 1 will increase convergence speed.]])
cmd:option('-pre_word_vecs_enc', 'word_embedding.hdf5', [[If a valid path is specified, then this will load
                                      pretrained word embeddings (hdf5 file) on the encoder side.
                                      See README for specific formatting instructions.]])
cmd:option('-pre_word_vecs_dec', 'word_embedding.hdf5', [[If a valid path is specified, then this will load
                                      pretrained word embeddings (hdf5 file) on the decoder side.
                                      See README for specific formatting instructions.]])
cmd:option('-fix_word_vecs_enc', 1, [[If = 1, fix word embeddings on the encoder side]])
cmd:option('-fix_word_vecs_dec', 1, [[If = 1, fix word embeddings on the decoder side]])
cmd:option('-max_batch_l', 256, [[If blank, then it will infer the max batch size from validation
...
cmd:option('-seed', 1331, [[Seed for random initialization]])
...
function train(train_data, valid_data)
    ...
    for epoch = opt.start_epoch, opt.epochs do
        ...
        local savefile = string.format('%s_epoch%s.t7', opt.savefile, epoch)
